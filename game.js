...
let safeTopOffset = 0
let uiOffsetX = 0
// 网页端按真实窗口宽度分类；画布仍维持固定最大宽度，避免桌面端横向拉伸。
let deviceType = 'mobile'
let uiScale = 1
let fontScale = 1

function updateUiLayout(info) {
  systemInfo = info || platform.getSystemInfoSync()
  screenWidth = systemInfo.windowWidth
  screenHeight = systemInfo.windowHeight
  pixelRatio = systemInfo.pixelRatio || 1
  const viewportWidth = systemInfo.viewportWidth || screenWidth
  deviceType = viewportWidth < 600 ? 'mobile' : (viewportWidth <= 1024 ? 'tablet' : 'desktop')
  uiScale = deviceType === 'desktop' ? (viewportWidth >= 1600 ? 0.7 : 0.8) : (deviceType === 'tablet' ? 0.9 : 1)
  fontScale = uiScale
  // 桌面端限制实际绘制宽度，居中展示，避免把手机设计稿强行撑满宽屏。
  const maxRenderScale = deviceType === 'desktop' ? 1.25 : (deviceType === 'tablet' ? 1.15 : screenWidth / DESIGN_WIDTH)
  scale = Math.min(screenWidth / DESIGN_WIDTH, maxRenderScale)
  viewHeight = screenHeight / scale
  const safeAreaTop = systemInfo.safeArea ? systemInfo.safeArea.top : 0
  safeTopOffset = Math.max(0, Math.ceil(safeAreaTop / scale - 16))
  uiOffsetX = Math.max(0, (screenWidth - DESIGN_WIDTH * scale) / 2)
}

function applyCanvasTransform() {
  context.setTransform(pixelRatio * scale, 0, 0, pixelRatio * scale, pixelRatio * uiOffsetX, 0)
}

function resizeCanvas() {
  updateUiLayout()
  canvas.width = Math.max(1, Math.round(screenWidth * pixelRatio))
  canvas.height = Math.max(1, Math.round(screenHeight * pixelRatio))
  drawGame()
}

updateUiLayout(systemInfo)
...
// 长文本详情页共用的临时滚动状态，不写入存档。
let techniqueScrollY = 0
let techniqueContentHeight = 0
let techniqueViewportHeight = 0
let longPageScrollY = 0
let longPageContentHeight = 0
let longPageViewportHeight = 0
let isTouchingLongPage = false
let longPageTouchLastY = 0
...
function drawAssetLoadingScreen() {
  ...
  context.font = 'bold ' + (22 * fontScale) + 'px sans-serif'
  context.textAlign = 'center'
  context.fillText('问道长生', DESIGN_WIDTH / 2, 180)
  context.font = 'normal ' + (13 * fontScale) + 'px sans-serif'
...
platform.onTouchStart(function (event) {
  ...
  const x = (touch.clientX - uiOffsetX) / scale
  ...
  if (!activeStoryDialog && isInLongPageContentArea(x, y)) {
    isTouchingLongPage = true
    longPageTouchLastY = y
  }
...
platform.onTouchMove(function (event) {
  ...
  const x = (event.touches[0].clientX - uiOffsetX) / scale
  ...
  if (isTouchingLongPage) {
    setCurrentLongPageScroll(getCurrentLongPageScroll() - (y - longPageTouchLastY))
    clampLongPageScroll()
    longPageTouchLastY = y
    drawGame()
  }
...
platform.onTouchEnd(function () {
  ...
  if (isTouchingLongPage) { isTouchingLongPage = false; clampLongPageScroll(); drawGame() }
...
// 鼠标滚轮与手机拖动使用同一套内容高度和边界判断。
platform.onWheel(function (event) {
  if (currentPage !== 'technique' && currentPage !== 'event' && !(currentPage === 'sect' && sectPage === 'tasks')) return
  setCurrentLongPageScroll(getCurrentLongPageScroll() + (event.deltaY || 0) / scale)
  clampLongPageScroll()
  drawGame()
})
...
function drawGame() {
  ...
  if (DEBUG_MODE) {
    const deviceName = deviceType === 'desktop' ? 'PC' : (deviceType === 'tablet' ? '平板' : '手机')
    drawText('设备：' + deviceName, 278, 39, 8, '#7a7467')
    drawText(screenWidth + '×' + screenHeight + '  UI×' + uiScale.toFixed(2), 226, 50, 8, '#7a7467')
  }
...
function drawStatusCard() {
  ...
  drawButton(250, 164, 96, 24, '功法详情', '#526d5b', function () { techniqueScrollY=0; currentPage='technique'; drawGame() })
...
function wrapTextLines(text,maxWidth,fontSize,fontWeight){const lines=[],content=String(text||'');let line='';context.save();context.font=(fontWeight||'normal')+' '+(fontSize*fontScale)+'px sans-serif';for(let i=0;i<content.length;i++){const test=line+content[i];if(context.measureText(test).width>maxWidth&&line){lines.push(line);line=content[i]}else line=test}if(line)lines.push(line);context.restore();return lines.length?lines:['']}
...
function drawCenteredTextInRect(text,x,y,w,h,size,color,weight){context.save();context.font=(weight||'normal')+' '+(size*fontScale)+'px sans-serif';context.fillStyle=color;context.textAlign='center';context.textBaseline='middle';context.fillText(text,x+w/2,y+h/2);context.restore()}
...
function isInLongPageContentArea(x,y) {
  const isScrollableSectTask = currentPage === 'sect' && sectPage === 'tasks'
  return (currentPage === 'technique' || currentPage === 'event' || isScrollableSectTask) && x >= 14 && x <= 361 && y >= 68 && y <= getNavigationY() - 8
}
function getCurrentLongPageScroll() {
  return currentPage === 'technique' ? techniqueScrollY : longPageScrollY
}
function setCurrentLongPageScroll(value) {
  if (currentPage === 'technique') techniqueScrollY = value
  else longPageScrollY = value
}
function clampLongPageScroll() {
  const max = Math.max(0, longPageContentHeight - longPageViewportHeight)
  setCurrentLongPageScroll(Math.max(0, Math.min(getCurrentLongPageScroll(), max)))
}
...
function drawSectPage(){refreshSectDailyTasks();updateSectTaskStates();if(sectPage==='tasks'){drawSectTasksPage();return}if(sectPage==='shop'){drawSectShopPage();return}if(sectPage==='library'){drawSectLibraryPage();return}if(sectPage==='forge'){drawSectForgePage();return}if(sectPage==='alchemy'){drawSectAlchemyPage();return}if(sectPage==='secret'){drawSectSecretPage();return}if(sectPage==='mansion'){drawMansionTab();drawSectBack();return}drawRoundedRect(14,68,347,155,10,'#fffdf7','#ded7c8');drawCenteredText('青云宗  Lv.'+gameData.sect.sectLevel,92,20,'#8b4513','bold');drawLabelValue('职位',getSectPosition().name,122);drawLabelValue('宗门贡献',formatNumber(gameData.sectContribution),148);drawLabelValue('宗门声望',formatNumber(gameData.sect.sectFavor),174);drawLabelValue('本轮任务',gameData.sect.dailyTasks.filter(function(t){return t.claimed}).length+' / '+gameData.sect.dailyTasks.length,200);const items=[['tasks','宗门任务'],['shop','宗门商店'],['library','藏经阁'],['forge','炼器阁'],['alchemy','丹阁'],['secret','宗门秘境'],['mansion','洞府'],['home','职位说明']];items.forEach(function(item,i){drawButton(20+(i%2)*175,240+Math.floor(i/2)*58,165,46,item[1],item[0]==='home'?'#737a70':'#456b78',function(){if(item[0]==='home')showPositionPrivileges();else{if(item[0]==='tasks')longPageScrollY=0;sectPage=item[0];drawGame()}})})}
function drawSectBack(){drawButton(105,getNavigationY()-48,165,36,'返回宗门','#737a70',function(){sectPage='home';drawGame()})}
function drawSectTasksPage(){
  const remain=Math.max(0,Math.ceil((gameData.sectTaskRefreshTime-Date.now())/1000))
  const top=68, bottom=getNavigationY()-8, returnTop=bottom-46, viewportTop=112, viewportBottom=returnTop-8
  drawText('宗门任务（每2小时刷新5个）',24,82,17,'#8b4513','bold')
  drawText('下次刷新：'+Math.floor(remain/60)+'分'+remain%60+'秒',24,101,11,'#8a857a')
  context.save(); context.beginPath(); context.rect(14,viewportTop,347,viewportBottom-viewportTop); context.clip()
  let y=118-longPageScrollY
  gameData.sect.dailyTasks.forEach(function(task){
    const description=task.description+'  '+Math.min(task.progress,task.target)+'/'+task.target
    const lines=wrapTextLines(description,225,11,'normal')
    const height=Math.max(66,45+lines.length*16)
    drawRoundedRect(14,y,347,height,8,'#fffdf7','#ded7c8')
    drawText(task.name,26,y+15,14,'#37474f','bold')
    drawWrappedText(description,26,y+33,225,16,11,'#625c50')
    drawText('贡献 '+task.contribution+'  灵石 '+task.stone+(task.material?'  玄铁精 '+task.material:''),26,y+height-14,11,'#8b4513')
    drawButton(267,y+16,78,30,task.claimed?'已领取':task.completed?'领取':'进行中',task.completed&&!task.claimed?'#526d5b':'#aaa398',function(){claimSectTask(task.id)})
    y+=height+8
  })
  context.restore()
  longPageContentHeight=y+longPageScrollY-viewportTop
  longPageViewportHeight=viewportBottom-viewportTop
  clampLongPageScroll()
  drawSectBack()
}
...
function drawTechniquePage() {
  ...
  const returnTop = bottom - 46
  // 标题固定在顶部，滚动内容从标题下方开始，避免文字滑到标题背后。
  const viewportTop = 112
  const viewportBottom = returnTop - 8
  const viewportHeight = viewportBottom - viewportTop
  drawRoundedRect(14,top,347,bottom-top,10,'#fffdf7','#ded7c8')
  drawCenteredText('当前功法',92,18,'#8b4513','bold')
  if (!t) {
    drawCenteredText('尚未获得可修炼功法',160,14,'#8a857a')
    drawButton(105,returnTop,165,34,'返回修炼','#737a70',function(){currentPage='cultivate';drawGame()})
    return
  }
  context.save(); context.beginPath(); context.rect(14,viewportTop,347,viewportHeight); context.clip()
  let y = 132 - techniqueScrollY
  const root = SPIRIT_ROOT_TYPES.find(function(item){return item.id===t.element}) || {}
  const effects=(t.effects||[])
  drawText(getTechniqueDisplayName(),27,y,18,'#8b4513','bold'); y+=28
  drawText('属性：'+(root.name||t.element),27,y,13,'#625c50'); y+=22
  drawText('等级：'+t.level,27,y,13,'#625c50'); y+=30
  drawTechniqueDivider(y); y+=22
  drawText('基础效果',27,y,15,'#37474f','bold'); y+=24
  drawText('修炼速度：+'+Math.round((t.baseRate||0)*100)+'%',27,y,13,'#625c50'); y+=21
  drawText('等级增幅：+'+Math.max(0,t.level-1)*2+'%',27,y,13,'#625c50'); y+=30
  drawTechniqueDivider(y); y+=22
  drawText('功法词条',27,y,15,'#37474f','bold'); y+=24
  if (!effects.length) { drawText('暂无额外词条',27,y,12,'#8a857a'); y+=24 }
  effects.forEach(function(effect,index){drawWrappedText('①②③④'.charAt(index)+' '+effect.name+'：+'+Math.round(effect.value*100)+'%',27,y,300,18,12,'#625c50'); y+=26})
  y+=8; drawTechniqueDivider(y); y+=22
  drawText('灵根适配',27,y,15,'#37474f','bold'); y+=24
  drawText((root.name||t.element)+'灵根：倍率×'+formatRate(getSpiritRootMultiplier()),27,y,13,'#625c50'); y+=30
  drawTechniqueDivider(y); y+=22
  const breakdown=getCultivationSpeedBreakdown()
  drawText('最终修炼效果',27,y,15,'#37474f','bold'); y+=25
  drawText('当前贡献：功法倍率×'+formatRate(getTechniqueCultivationRate()),27,y,13,'#8b4513','bold'); y+=28
  drawText('修炼速度组成',27,y,14,'#37474f','bold'); y+=22
  let lineY=y
  ...
  y=lineY+10
  drawText('最终修炼倍率：×'+formatRate(breakdown.total),27,y,15,'#8b4513','bold'); y+=28
  drawTechniqueDivider(y); y+=22
  drawText('功法介绍',27,y,15,'#37474f','bold'); y+=23
  const intro=(root.name||'此')+'系功法与灵根相合，当前倍率已用于主动修炼、聚灵阵自动修炼与离线自动修炼。'
  const introLines=wrapTextLines(intro,300,12,'normal')
  drawWrappedText(intro,27,y,300,18,12,'#625c50'); y+=introLines.length*18+18
  context.restore()
  techniqueContentHeight = y + techniqueScrollY - viewportTop
  techniqueViewportHeight = viewportHeight
  longPageContentHeight = techniqueContentHeight
  longPageViewportHeight = techniqueViewportHeight
  clampLongPageScroll()
  drawButton(105,returnTop,165,34,'返回修炼','#737a70',function(){currentPage='cultivate';drawGame()})
}

function drawTechniqueDivider(y) {
  context.save(); context.strokeStyle='#ded7c8'; context.lineWidth=1
  context.beginPath(); context.moveTo(27,y); context.lineTo(348,y); context.stroke(); context.restore()
}
...
function drawRandomEventPage() {
  ...
  const top=68, bottom=getNavigationY()-8, viewportTop=104, viewportBottom=bottom-8
  drawRoundedRect(14, top, 347, bottom-top, 10, '#fffdf7', '#ded7c8')
  drawCenteredText('机缘事件：' + event[1], 100, 18, '#8b4513', 'bold')
  context.save(); context.beginPath(); context.rect(14,viewportTop,347,viewportBottom-viewportTop); context.clip()
  let y=136-longPageScrollY
  const descriptionLines=wrapTextLines(event[2],320,14,'normal')
  drawWrappedText(event[2], 28, y, 320, 20, 14, '#625c50')
  y+=descriptionLines.length*20+22
  for (let i = 3; i < event.length; i++) {
    const choice = event[i]
    drawButton(28, y, 319, 42, choice[0], '#456b78', function () { resolveRandomEvent(choice[1]) })
    y+=56
  }
  context.restore()
  longPageContentHeight=y+longPageScrollY-viewportTop
...
function showCurrentTechniqueDetail(){if(!gameData.currentTechnique){showMessage('当前没有功法');return}techniqueScrollY=0;currentPage='technique';drawGame()}
...
function drawText(text, x, y, size, color, weight) {
  ...
  context.font = (weight || 'normal') + ' ' + (size * fontScale) + 'px sans-serif'
...
function drawLabelValue(label, value, y) {
  ...
  context.font = 'bold ' + (14 * fontScale) + 'px sans-serif'
...
function drawButton(x, y, width, height, text, color, onTap, onLongPress) {
  ...
  context.font = 'bold ' + (13 * fontScale) + 'px sans-serif'
  context.textAlign = 'center'
  context.textBaseline = 'middle'
...
function drawWrappedText(text, x, y, maxWidth, lineHeight, size, color) {
  context.fillStyle = color
  context.font = 'normal ' + (size * fontScale) + 'px sans-serif'
  context.textAlign = 'left'
  context.textBaseline = 'middle'
